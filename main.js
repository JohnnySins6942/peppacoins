...
function onReady(callback) {
  ...
  var empty = "";
    ...
    function InitializeReferrals()
    {
        ...
        firebase.auth().onAuthStateChanged(user => {
            ...
            docRef.get().then((doc) => {
                ...
                document.getElementById("MultiplierValue").innerHTML = "Multiplier: " + data.Multiplier + "x";
                var j = 0;
    ...
    function SendPeppa(){
        ...
        var NetworkFee = (Number(TransactionAmount.value) * 0.01);
                    ...
                    docRef.get().then((Senddoc) => {
                        if (Senddoc.exists) {
                            ...
                            if (coins >= Number(TransactionAmount.value) + Number(NetworkFee)){
                                var tempCoin = coins - (Number(TransactionAmount.value) + Number(NetworkFee));  
                                ...
                                db.collection("users").doc(Auth.currentUser.uid).set(tempData).then(() => {
                                    alert("You sent " + TransactionAmount.value + " PeppaCoins with a Network fee cost of " + NetworkFee.toString() +  " PEPPAS to the address " + receiverAddress.value + " successfully!")
                                ...
                                docRef.get().then((Receivedoc) => {
                                    if (Receivedoc.exists) {
                                        ...
                                        var ReceivetempCoin = Number(Rcoins) + Number(TransactionAmount.value);  
                                        var tempReceiveData = {
    ...
    function Mining(){
        ...
        docRef.get().then((doc) => {
            if (doc.exists) {
                ...
                var RewardSpeed = blocksMined * 5 + Math.floor(Math.random() * 1000);
                var Reward =  (Math.random() * 15)/2 * data.Multiplier;
            ...
            if (width >= 100) {
              ...
              docRef.get().then((doc) => {
                  if (doc.exists) {
                      var data = doc.data();
                      var docData = {
                        Coins: Number(data.Coins) + Number(reward),
                        ...
                        db.collection("users").doc(Auth.currentUser.uid).set(docData).then(() => {
                            isMining = false;
                            alert("You finished mining and gained " + reward.toString()) + " PeppaCoins";
                            InitializeWallet(Auth.currentUser);
    ...
    function InitializeWallet(auth){
        ...
        docRef.get().then((doc) => {
            if (doc.exists) {
                ...
                document.getElementById("Coins").innerHTML = coins.toFixed(4);
                ...
                for (var i = arrayLength - 1; i >= 0; i--) {
                    ...
                    if(transactions[i].send == true)
                    {
                        ...
                        <li>
                        <div style="background-color:pink;width:600px;border:15px solid red;padding:40px;margin:20px;">
                        <h2 id="TransactionIndex">Transaction Number ${(i+1).toString()} (Sent Transaction)</h2>
                        <h3 id="TransactionAmount">Sent Amount: -${transactions[i].Amount.toString()} PEPPA</h3>
                        ...
                        <h3 id="TransactionTime">NetworkFee: ${transactions[i].NetworkFee.toString()} PEPPA</h3> 
                        </div>'
                        </li>
                        `;
                        html += li;
                        ...
                        <li>
                        <div style="background-color:pink;width:600px;border:15px solid red;padding:40px;margin:20px;">
                        <h2 id="TransactionIndex">Transaction Number ${(i+1).toString()} (Received Transaction)</h2>
                        <h3 id="TransactionAmount">Received Amount: +${transactions[i].Amount.toString()} PEPPA</h3>
                ...
                var docRef = db.collection("users").doc(doc.id);
                                docRef.get().then((Receivedoc) => {
                                    if (Receivedoc.exists) {
                                        ...
                                        var transactionAmount = (Math.random() * 5)/2 * Rdata.Multiplier;
                                        if(Date.parse(date1) > Date.parse(date2)){
                                        ...
                                        var ReceiveTransactionObj = {
                                            ReceiverAddress: receiverAddress.value,
                                            SenderAddress: "Peppa_6xujUqtgVPbWNYiI78QhAsBpKvRRluYVyRUTSXFCdXGA",
                                            TransactionDate: new Date(),
